...
public interface OdsTravelerRepository extends JpaRepository<OdsTraveler, Integer> {
    ...
    boolean existsByTuuid(@Param("tuuid") String tuuid);

    boolean existsByDocumentNumber(@Param("document_number") String documentNumber);

    OdsTraveler findFirstByDocumentNumber(@Param("document_number") String documentNumber);

    @Query(value = "SELECT * FROM odysseus_bk.ods_travelers WHERE document_number = :document_number ORDER BY id DESC LIMIT 1", nativeQuery = true)
    OdsTraveler findLastByDocumentNumberNative(@Param("document_number") String documentNumber);

    boolean existsByLicensePlate(@Param("license_plate") String LicensePlate);

    OdsTraveler findFirstByLicensePlate(@Param("license_plate") String LicensePlate);
