...
The project is used to offer the functionalities for the trader application
## Getting Started

These instructions will get you a copy of the project up and running on your local machine for development and testing purposes. See deployment for notes on how to deploy the project on a live system.
On a local machine, just get the jar file and execute it 
For example java -jar odysseus-traveler-0.0.1-SNAPSHOT.jar
...
java -Dspring.profiles.active=test -jar odysseus-traveler-0.0.1-SNAPSHOT.jar

To build the docker image, follow the steps:
1. Copy docker/Dockerfile in the root folder of the system. 
2. Be sure that under the root there is target/odysseus-traveler-0.0.1-SNAPSHOT.jar
3. Run the command docker build -t hometra . (be careful to use dot at the end)
4. Wait the image to be created.

To run the docker image
1. Run the docker command: docker run -d hometra
2. 
...
test profile is using keycloack installed on the platform (like 10.129.150.116)
production profile is using keycloack installed on SIMAVI data lake environment
...
To save the image
1. Run docker save, for example docker image save -o homeapp.tar hometra
## Running the tests
...
2. Copy the jar file in a chosen location. For example copy odysseus-traveler-0.0.1-SNAPSHOT.jar in /tmp/ folder
3. Go to the folder where the jar file is. For example cd /tmp
4. Run java -jar command. Use in command line the appropriate profile. For example java -jar odysseus-traveler-0.0.1-SNAPSHOT.jar
If you want to use a specific profile, enter it in the command line
For example to use the test profile, just use the command
java -Dspring.profiles.active=test -jar odysseus-traveler-0.0.1-SNAPSHOT.jar
...
To run the docker image
1. Verify that the image exists. Run docker image ls and search the hometra image
2. Run the docker command: docker run -d -p 26099:6099 hometra
